...
export async function GET(request){
 ...
 try{
  // Direct profile/document routes are passed through exactly; no batch filters or duplicate suppression.
  if(!path.startsWith('/v1.0/powerquery/')){
   const{response,data}=await upstreamFetch(base,key);
   return NextResponse.json({ok:response.ok,status:response.status,source:'importyeti',data},{status:response.status,headers:{'Cache-Control':'no-store'}});
  }
